...
function root(args) {
  ...
  console.log(`in root ${args}`);
  console.log(`in root ${path.join.apply(path, [__dirname].concat(args))}`);
...
function rootNode(args) {
  args = Array.prototype.slice.call(arguments, 0);
  console.log(`in rootNode ${args}`);
  console.log(`in rootNode ${root.apply(path, ['node_modules'].concat(args))}`);  
